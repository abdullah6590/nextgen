    ...
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className="relative w-full max-w-md h-full bg-surface border-l border-[rgba(255,255,255,0.05)] shadow-[-10px_0_30px_rgba(0,0,0,0.5)] flex flex-col transform transition-transform duration-300">
        <div className="p-6 border-b border-[rgba(255,255,255,0.05)] flex justify-between items-center">
          <h2 className="text-xl font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-neonBlue to-neonPurple">
            Inventory Cache
          </h2>
          <button 
            onClick={() => setCartOpen(false)}
            className="text-textSub hover:text-neonPink transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-textSub opacity-50">
            ...
            items.map((item) => (
              <div key={item.productId} className="flex gap-4 border border-[rgba(255,255,255,0.05)] bg-surfaceHover/50 p-4 rounded">
                <div className="w-20 h-20 bg-background rounded border border-[rgba(255,255,255,0.05)] flex items-center justify-center overflow-hidden">
                   {item.image ? (
                     ...
                     <span className="text-neonBlue opacity-30 font-mono text-xs">IMG_NA</span>
                   )}
                </div>
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-bold text-sm text-white line-clamp-1">{item.name}</h3>
                    <p className="text-neonBlue font-mono text-sm">${item.price.toFixed(2)}</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 bg-background rounded px-2 py-1 border border-[rgba(255,255,255,0.05)]">
                      <button onClick={() => updateQuantity(item.productId, item.quantity - 1)} className="text-textSub hover:text-white">-</button>
                      <span className="text-xs font-mono w-4 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.productId, item.quantity + 1)} className="text-textSub hover:text-white">+</button>
                    </div>
                    <button 
                      onClick={() => removeFromCart(item.productId)}
                      className="text-textSub hover:text-neonPink text-xs uppercase tracking-wider transition-colors"
                    >
                      Delete
                    </button>
                  </div>
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-[rgba(255,255,255,0.05)] bg-surfaceHover/30">
            <div className="flex justify-between items-center mb-6">
              <span className="text-textSub uppercase tracking-widest text-sm">Total Value</span>
              <span className="text-neonBlue font-mono text-xl font-bold">${totalPrice.toFixed(2)}</span>
            </div>
